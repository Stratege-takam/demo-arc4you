...
[Export, Scoped]
public class DemoPCBE99925ManageCourseServiceParticipantFacade
{
    public DemoPCBE99925ManageCourseServiceParticipantFacade(IOptionsMonitor<SimpleKeyValueSettings> optionsSettings, IHttpClientFactory httpClientFactory)
    {
        ParticipantClient.PreProcessException = (response) =>
        {
        ...
        if (tokenSettings is not null)
        {
            ...
            Proxy = new ParticipantClient(client)
            {
    ...
    public ParticipantClient? Proxy { get; private set; }
}